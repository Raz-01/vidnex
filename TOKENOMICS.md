...
1. **Free social is always free.** Watch, like, comment, follow, share —
   never cost a token.
...
- Any balance shown in the UI (e.g. `creators.tokenBalance`) is a cache —
  the ledger table is the single source of truth.
